...
export default function CompaniesPage() {
  ...
  const [activeTab, setActiveTab] = useState<"providers" | "seekers">(
    "providers",
  );
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredCompanies.map((company) => (
            ...
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 ${activeTab === "providers" ? "bg-primary/20" : "bg-purple-500/20"} rounded-bl-full -mr-16 -mt-16 transition-transform group-hover:scale-150`}
              />
        ...
        <div className="text-center py-20 glass rounded-[3rem]">
          ...
          <p className="text-muted-foreground">
            Prova ad affinare la ricerca o cambia categoria.
          </p>
        </div>
      )}
    </div>
